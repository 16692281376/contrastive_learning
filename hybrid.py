...
def train_supconce(encoder, projector, classifier, train_loader, val_loader, train_transform, val_transform, criterion, criterion_sl, optimizer, epochs, scheduler):
    ...
    logger = get_logger(logpath="logs.txt")
    logger.info(encoder)
    ...
    logger.info(f"best icbhi score is {format(best_icbhi_score, '.4f')} (se:{format(best_se, '.4f')} sp:{format(best_sp, '.4f')}) at epoch {best_epoch_icbhi}")
